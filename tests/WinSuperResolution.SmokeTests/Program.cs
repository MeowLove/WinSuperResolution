...
namespace WinSuperResolution.SmokeTests
{
    internal static class Program
    {
        ...
        private static void TestEmbeddedLocalization()
        {
            ...
            Assert(Strings.ForCulture("en-US")["CompatibilityOtherGraphics"] == "Other detected adapters", "English compatibility detail label is missing.");
            ...
            Assert(Strings.ForCulture("en-US")["HomeTab"] == "1. Home", "English home tab label is missing.");
            Assert(Strings.ForCulture("zh-CN")["SuperResolutionTab"] == "2. 超分", "Chinese super-resolution tab label is missing.");
            Assert(Strings.ForCulture("ru-RU")["ScalingTab"] == "3. Масштабирование", "Russian scaling tab label is missing.");
            Assert(Strings.ForCulture("en-US")["HomeTabDescription"] == "Check compatibility and recover", "English home tab description is missing.");
            Assert(Strings.ForCulture("zh-CN")["ScalingTabDescription"] == "重启后设置分辨率与缩放", "Chinese scaling tab description is missing.");
            Assert(Strings.ForCulture("ru-RU")["SuperResolutionTabDescription"] == "Запись возможностей виртуального разрешения", "Russian super-resolution tab description is missing.");
            Assert(Strings.ForCulture("zh-CN")["SuperResolutionWorkflow"].Contains("立即重启 Windows"), "Chinese super-resolution workflow guidance is missing.");
            Assert(Strings.ForCulture("zh-CN")["SuperResolutionTroubleshootingDetails"].Contains("最终修复"), "Chinese display-cache final-repair guidance is missing.");
            Assert(Strings.ForCulture("zh-CN")["ScalingWorkflow"].Contains("显示设置"), "Chinese scaling fallback guidance is missing.");
