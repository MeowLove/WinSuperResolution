    ...
    internal static class Strings
    {
        ...
        private static IDictionary<string, string> CreateEnglish()
        {
            return Create(
                ...
                "Ready", "Ready. Refresh to scan registered display configurations.", "Recovery", "Recovery and operation history", "RecoveryLocation", "Recovery files are kept here", "OpenRecoveryFolder", "Open recovery folder", "NoOperationYet", "No operation has been recorded in this session.", "RestartRequired", "Restart required", "DisplayRecordsHint", "Exact association is required only for direct current-mode or scale changes. Registry capability planning remains available for eligible registered records.");
        }
        ...
        private static IDictionary<string, string> CreateChinese()
        {
            return Create(
                ...
                "Ready", "已就绪。刷新以扫描已注册的显示配置。", "Recovery", "恢复与操作记录", "RecoveryLocation", "恢复文件保存在此处", "OpenRecoveryFolder", "打开恢复目录", "NoOperationYet", "本次会话尚未执行操作。", "RestartRequired", "需要重启", "DisplayRecordsHint", "仅直接修改当前分辨率或缩放时需要 Exact 关联；对符合条件的已注册记录，仍可生成注册表能力计划。" );
        }
        ...
        private static IDictionary<string, string> CreateRussian()
        {
            return Create(
                ...
                "Ready", "Готово. Обновите, чтобы просканировать зарегистрированные конфигурации дисплея.", "Recovery", "Восстановление и история операций", "RecoveryLocation", "Файлы восстановления хранятся здесь", "OpenRecoveryFolder", "Открыть папку восстановления", "NoOperationYet", "В этом сеансе операций пока не было.", "RestartRequired", "Требуется перезапуск", "DisplayRecordsHint", "Связь Exact нужна только для прямого изменения текущего режима или масштаба. Планирование возможностей реестра остаётся доступным для подходящих зарегистрированных записей.");
        }
