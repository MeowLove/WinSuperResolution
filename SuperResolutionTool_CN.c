...
HINSTANCE hInst;
HWND hMainWnd;
HWND hMonitorComboBox;       // 显示器选择下拉框
HWND hScaleTrackbar;         // 缩放比例拖动条
HWND hNewResolutionLabel;    // 新分辨率标签
HWND hDesktopResolutionLabel; // 桌面分辨率标签
HWND hActiveResolutionLabel;  // 活动信号分辨率标签
HWND hMagnificationLabel;    // 缩放比例标签
HWND hDisplaySettingsButton; // 显示设置按钮
HWND hRebootButton;          // 重启按钮
HWND hBackupButton;          // 备份按钮
HWND hApplyButton;           // 应用按钮
HWND hAboutButton;           // 关于按钮

WCHAR selectedMonitor[256] = L""; // 当前选择的显示器
int selectedMonitorIndex = 0;
int magnification = 150;        // 缩放比例，默认 150%
int originalDesktopWidth = 0;   // 原始桌面分辨率宽度
int originalDesktopHeight = 0;  // 原始桌面分辨率高度
int activeSignalWidth = 0;      // 活动信号分辨率宽度
int activeSignalHeight = 0;     // 活动信号分辨率高度
int newDesktopWidth = 0;        // 新桌面分辨率宽度
int newDesktopHeight = 0;       // 新桌面分辨率高度
...
void PositionButtons(HWND hwnd); // 按钮定位函数
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow) {
    hInst = hInstance;
    hMainWnd = CreateWindowEx(0, L"SuperResolutionToolClass", L"显示器超分辨率工具 v1.0",
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
    case WM_CREATE: {
        CreateWindow(L"STATIC", L"全局“超分辨率，超级显示缩放”", WS_VISIBLE | WS_CHILD | SS_LEFT, 20, 10, 440, 20, hwnd, NULL, hInst, NULL);
        CreateWindow(L"STATIC", L"选择显示器:", WS_VISIBLE | WS_CHILD | SS_LEFT, 20, 40, 100, 20, hwnd, NULL, hInst, NULL);
        ...
        CreateWindow(L"STATIC", L"缩放比例:", WS_VISIBLE | WS_CHILD | SS_LEFT, 20, 70, 100, 20, hwnd, NULL, hInst, NULL);
        ...
        CreateWindow(L"STATIC", L"新桌面分辨率:", WS_VISIBLE | WS_CHILD | SS_LEFT, 20, 110, 120, 20, hwnd, NULL, hInst, NULL);
        hNewResolutionLabel = CreateWindow(L"STATIC", L"", WS_VISIBLE | WS_CHILD | SS_LEFT, 150, 110, 200, 20, hwnd, NULL, hInst, NULL);
        CreateWindow(L"STATIC", L"当前桌面分辨率:", WS_VISIBLE | WS_CHILD | SS_LEFT, 20, 140, 130, 20, hwnd, NULL, hInst, NULL);
        hDesktopResolutionLabel = CreateWindow(L"STATIC", L"", WS_VISIBLE | WS_CHILD | SS_LEFT, 150, 140, 200, 20, hwnd, NULL, hInst, NULL);
        CreateWindow(L"STATIC", L"活动信号分辨率:", WS_VISIBLE | WS_CHILD | SS_LEFT, 20, 170, 130, 20, hwnd, NULL, hInst, NULL);
        hActiveResolutionLabel = CreateWindow(L"STATIC", L"", WS_VISIBLE | WS_CHILD | SS_LEFT, 150, 170, 200, 20, hwnd, NULL, hInst, NULL);

        // 创建按钮，但不设置位置
        hDisplaySettingsButton = CreateWindow(L"BUTTON", L"显示设置", WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 0, 0, 140, 30, hwnd, (HMENU)1, hInst, NULL);
        hRebootButton = CreateWindow(L"BUTTON", L"重启", WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 0, 0, 120, 30, hwnd, (HMENU)2, hInst, NULL);
        hBackupButton = CreateWindow(L"BUTTON", L"备份", WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 0, 0, 120, 30, hwnd, (HMENU)3, hInst, NULL);
        hApplyButton = CreateWindow(L"BUTTON", L"应用", WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 0, 0, 120, 30, hwnd, (HMENU)4, hInst, NULL);
        hAboutButton = CreateWindow(L"BUTTON", L"关于 与 帮助", WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 0, 0, 140, 30, hwnd, (HMENU)5, hInst, NULL);
        ...
        PositionButtons(hwnd); // 在 WM_CREATE 中调用一次
        break;
    }
    case WM_COMMAND: {
        ...
        switch (wmId) {
            ...
            case 4:
                if (ApplyResolutionChanges()) {
                    int msgboxID = MessageBox(hwnd, L"注册表已更新。重启电脑以使更改生效。\n是否立即重启？", L"成功", MB_YESNO | MB_ICONINFORMATION);
...
void PopulateMonitorList() {
    ...
    if (status != ERROR_SUCCESS) {
        DisplayErrorMessage(L"无法打开注册表项: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers\\Configuration", L"错误");
...
void BackupRegistry() {
    ...
    if (GetCurrentDirectory(MAX_PATH, currentDir) == 0) {
        DisplayErrorMessage(L"获取当前目录失败。", L"错误");
    ...
    if (CreateProcessW(NULL, command, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi)) {
        ...
        StringCchPrintf(message, MAX_PATH + 100, L"注册表备份成功:\n%s", fullPath);
        MessageBox(hMainWnd, message, L"备份成功", MB_OK | MB_ICONINFORMATION);
        DebugPrint(L"Registry backup created successfully.\n");

    } else {
        DWORD errorCode = GetLastError();
        WCHAR errorMsg[256];
        StringCchPrintf(errorMsg, 256, L"创建备份失败。错误代码: %lu", errorCode);
        MessageBox(hMainWnd, errorMsg, L"备份错误", MB_ICONERROR | MB_OK);
        DebugPrint(L"Failed to create backup. Error code: %lu\n", errorCode);
    }
}
BOOL ApplyResolutionChanges() {
     if (wcslen(selectedMonitor) == 0) {
        MessageBox(hMainWnd, L"请选择一个显示器。", L"错误", MB_OK | MB_ICONERROR);
        return FALSE;
    }
    ...
    int result = MessageBox(hMainWnd, confirmMessage, L"确认更改分辨率", MB_YESNO | MB_ICONWARNING);
    ...
    if (status != ERROR_SUCCESS) {
         ...
         StringCchPrintf(errorMessage, 256, L"打开注册表项失败: %s, 错误代码: %lu", regPath, GetLastError());
         DisplayErrorMessage(errorMessage, L"错误");
    ...
    if (status != ERROR_SUCCESS) {
         WCHAR errorMessage[256];
         StringCchPrintf(errorMessage, 256, L"写入 PrimSurfSize.cx 失败: %s. 错误代码: %lu", regPath, GetLastError());
        RegCloseKey(hKey);
        DisplayErrorMessage(errorMessage, L"错误");
    ...
    if (status != ERROR_SUCCESS) {
         WCHAR errorMessage[256];
         StringCchPrintf(errorMessage, 256, L"写入 PrimSurfSize.cy 失败: %s. 错误代码: %lu", regPath, GetLastError());
        RegCloseKey(hKey);
        DisplayErrorMessage(errorMessage, L"错误");
...
void ShowAboutDialog() {
     MessageBox(hMainWnd,
        ...
        L"版本: v1.0\n"
        L"作者: CXT\n"
        L"网站: https://www.cxthhhhh.com\n\n"
        L"全局“超分辨率，超级显示缩放”，类似于 NVIDIA DLSS、AMD FSR、Intel XeSS 和 Microsoft DirectSR。\n"
        L"在保持输出信号分辨率不变的情况下，增加屏幕上显示的内容。\n\n"
        L"使用方法：\n"
        L"1. 以管理员身份运行 SuperResolutionTool.exe。\n"
        L"2. 从下拉列表中选择要修改的显示器。\n"
        L"3. 使用拖动条调整缩放比例。将显示新的桌面分辨率。\n"
        L"4. （强烈建议）单击“备份”按钮以保存当前注册表设置。\n"
        L"5. 单击“应用”按钮。将出现一个确认对话框。查看更改并单击“是”继续。\n"
        L"6. 重新启动计算机。您也可以单击“重启”按钮。\n"
        L"7. 重新启动后，转到 Windows 设置 -> 系统 -> 显示，选择修改后的显示器，然后从“显示分辨率”下拉列表中选择新的分辨率。\n"
        L"   您也可以单击“显示设置”以快速打开它。\n\n"
        L"警告：修改显示设置可能会导致显示问题。在应用更改之前，请务必备份您的注册表。",
        L"关于 & 帮助",
...
void RestartComputer() {
    int msgboxID = MessageBox(hMainWnd, L"您确定要重新启动计算机吗？", L"确认重启", MB_YESNO | MB_ICONQUESTION);
    ...
    if (msgboxID == IDYES) {
        ...
        if (!OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken)) {
            DisplayErrorMessage(L"OpenProcessToken 失败。", L"错误");
        ...
        if (GetLastError() != ERROR_SUCCESS) {
             DisplayErrorMessage(L"AdjustTokenPrivileges 失败。", L"错误");
        ...
        if (!ExitWindowsEx(EWX_REBOOT | EWX_FORCE, SHTDN_REASON_MAJOR_OPERATINGSYSTEM | SHTDN_REASON_MINOR_RECONFIG)) {
            DisplayErrorMessage(L"ExitWindowsEx 失败。", L"错误");
...
void OpenDisplaySettings() {
    ...
    if ((intptr_t)result <= 32) {
        DisplayErrorMessage(L"打开显示设置失败。", L"错误");
